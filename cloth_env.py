...
class ClothEnv(gym.Env):
    ...
    def __init__(self, cfg_file, index, main_data_save_folder, subrank=None, start_state_path=None): #'/home/rpklab/fabric-vsf/blender_data/cloth_state.pkl'):
        ...
        self.folder = main_data_save_folder + 'episode_' + str(index)
        if not os.path.exists(self.folder):
            os.makedirs(self.folder)
        if index < 3500: 
            self._init_type = 'tier1' 
        elif index > 3500 and index < 7000: 
            self._init_type = 'tier2'
        else: 
            self._init_type = 'tier3'
    ...
    def get_blender_rep(self, use_depth):
        ...
        frame_path = '/home/rpklab/fabric-vsf/blender_data/frame0.obj'
        floor_path = '/home/rpklab/fabric-vsf/blender_data/floor.obj'
        ...
        if sys.platform == 'darwin':
            ...
            subprocess.call([
                ...
                # '/home/rpklab/blender-2.93.2-linux-x64/blender', '--background', '--python', bfile, '--', tm_path,
                '/home/rpklab/blender-2.79-linux-glibc219-x86_64/blender', '--background', '--python', bfile, '--', tm_path,
                str(self._hd), str(self._wd), str(init_side), self._init_type,
                frame_path, self._oracle_reveal, use_depth, floor_path,
                self.__add_dom_rand, ",".join([str(i) for i in self.dom_rand_params['c']]),
                ",".join([str(i) for i in self.dom_rand_params['n1']]),
    ...
    def step(self, action, initialize=False):
        ...
        pts = np.array([[p.x, p.y, p.z] for p in self.cloth.pts]) #self.cloth.pts
        info = {
    ...
    def render(self, filepath, mode='matplotlib', close=False):
...
def run_episode(cfg_file, index, main_data_save_folder): 
    '''
    This function run an episode of implementing the action to the cloth
    '''
    env = ClothEnv(cfg_file, index, main_data_save_folder)
    state_dict = dict({'obs_rgb': [], 'obs_depth': [], 'act': [], 'res_rgb': [], 'res_depth': [], 'pts': []})
    obs = env.reset()
    pts = np.array([[p.x, p.y, p.z] for p in env.cloth.pts]) 
    state_dict['pts'].append(pts)
    ...
    while num_steps < max_actions:
        print('now is running the ', str(num_steps), 'th step...')
        action = env.get_random_action(atype='over_xy_plane')
        state_dict['obs_rgb'].append(obs[:, :, 0:3])
        state_dict['obs_depth'].append(obs[:, :, 3])
        state_dict['act'].append(action)
        # Apply the action.
        obs, rew, done, info = env.step(action)
        state_dict['res_rgb'].append(obs[:, :, 0:3])
        state_dict['res_depth'].append(obs[:, :, 3])
        state_dict['pts'].append(info['pts'])
        num_steps += 1

    if env.render_proc is not None:
        env.render_proc.terminate()
        env.cloth.stop_render()
    state_file = env.folder + '/state_dict.npy'
    np.save(state_file, state_dict) 
...
if __name__ == "__main__":
    ...
    result_pkl = 'demos-{}-pol-{}.pkl'.format(date, 'vismpc')

    cfg_file = join(fp, '../../cfg/sim.yaml')
    render_path = join(fp, '../../render/build')    # Must be compiled!
    result_path = join(fp, '../../logs/{}'.format(result_pkl))

    today = datetime.datetime.today().strftime('%Y-%m-%d')
    main_data_save_folder = '/home/rpklab/DP_DATA/frabic_VSF/' + today + '/'
    if not os.path.exists(main_data_save_folder):
        os.makedirs(main_data_save_folder)
    
    # index = 0
    # run_episode(cfg_file, index, main_data_save_folder)
    ...
    for i in range(episode_num): 
        iterable.append((cfg_file, i, main_data_save_folder))
    pool.starmap(run_episode, iterable)
